...
- **`crates/domain`**: Pure business logic. Zero internal dependencies. Value objects enforce invariants at construction (e.g., `Email::new()` validates format). Newtypes use private inner fields with `as_str()` accessors.
- **`crates/application`**: Ports (Traits) and Use Cases. Uses native async traits (no `async-trait` crate).
- **`crates/infra`**: SQLx + SQLite adapters + Migrations (`/migrations`).
- **`crates/api`**: Axum server + configurable CORS + Tracing (Web Adapter). Uses `axum::extract::State` for compile-time safe state sharing.
- **`crates/cli`**: Clap-based CLI tool (Terminal Adapter).

Dependency versions are centrally managed via `[workspace.dependencies]` in `backend/Cargo.toml`.
...
|---|---|---|
| `DATABASE_URL` | `sqlite:local.db` | SQLite database path |
| `PORT` | `3001` | Backend API port |
| `CORS_ORIGIN` | `http://localhost:3000` | Allowed CORS origin (frontend URL) |
| `RUST_LOG` | `api=debug,tower_http=debug` | Tracing filter |
